...
2. 修改任何已存在的文件之前，**必须**先 read_file 读取原文。write_file 是整体覆盖语义，禁止凭猜测重写。
...
8. 长内容分片写：单次 write_file 的内容控制在 300 行以内。更长的文件先 write_file 写第一部分，
   再用 `append=true` 逐段追加。一次性输出超长内容容易被长度上限截断，
   导致工具调用的 JSON 参数不完整（例如只剩 content 而丢了 path）。
