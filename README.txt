...
9. 可观测：流式输出、每步打印思考/调用/回执/耗时；会话存 JSONL。
10. 可审阅性：/diff 看本次会话 unified diff；rollback 支持单文件回退。
11. 自主性增强：自动识别项目画像注入提示词；plan 工具复杂任务先列计划再执行；一轮内只读调用并行。
...
13. 贴近商业 code agent：在 todo/read_many_files/全局替换/web_fetch/recall/受控 git/think 之上，
   又补了权限系统、可干预、子任务编排等常规能力。
14. 跨会话项目记忆：memory 落盘 .minicode/memory.md 约定/踩坑/选型，启动注入提示词
    （类比 CLAUDE.md），免从零探索。
...
19. 后台命令：run_command 传 background=true 立即返回 job_id 不阻塞，用 check_command 读输出、
    kill_command 终止，可后台起服务/跑长任务，边等边干别的（类比 Claude Code 后台 shell）。
20. 自我改进：任务结束把失败/修复信号沉淀成经验，落盘 .minicode/memory.md
    （与 memory 同文件、标 [auto]、去重封顶），形成"失败→记忆→更聪明"闭环
    （类比 Claude Code 记忆自更新）；agent 也可中途 digest/list/forget。
