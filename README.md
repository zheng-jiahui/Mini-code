...
| 能力 | 说明 |
|---|---|
| 工具（共 27 个） | 文件：`read_file` `write_file` `edit_block` `list_dir` `read_many_files` `replace_in_file` `move_file` `copy_file` `delete` `replace_in_files`（跨文件替换）；检索：`grep_search`（支持 context 上下文）`find_files` `web_fetch` `recall`（相关文件检索）；执行：`run_command`；检查：`lint`（代码检查）；版本控制：`git`（受控提交 + 只读 + add）；控制：`finish` `ask_user` `plan` `todo`（任务清单）`think`（推理便签）`memory`（项目记忆）；汇报：`summary`（会话改动概览） |
